...
// Initialize PostgreSQL Pool and Prisma Client
const pool = new Pool({
  connectionString,
...
// Auto-initialize default schema if DB is accessible
...
// Register new user using Prisma Client
app.post('/api/register', async (req, res) => {
  const { full_name, email, username, role, bio } = req.body;
  ...
  try {
    ...
    res.status(201).json({
      message: 'Registration successful via Prisma!',
      user: {
        id: newUser.id,
